...
![Homepage Flow](/assets/everlaw-idea3.jpg)

I disliked the previous layout because there's no set place for "Favorited" items to live, which is a feature we aimed to push since our users tend to work on one card at a time. I tried a sidebar navigation homepage that defaults to the most recent cards you've accessed, with a tab dedicated to Favorites. While the sidebar approach looks much cleaner, I also tried defaulting to Favorites, but that still posed the same issues.


In the end, we took out the personal review stats because it takes up too much vertical real estate that is so crucial for users to pick up right where they left. I originally wanted them there for personalization, but focusing on the right work is even more valuable and personal.
{{ end-block }}
{{ end-block }}
